# FitResSummaryExamples

Python-native tutorial page for `FitResSummaryExamples`.

## Notebook
- [FitResSummaryExamples.ipynb](https://github.com/cajigaslab/nSTAT-python/blob/main/notebooks/FitResSummaryExamples.ipynb)
- Execution group: `full`

## Linked references
- [Examples index](../examples_index.md)
- [Class definitions](../class_definitions.md)
- [Paper overview](../paper_overview.md)
- DOI: `10.1016/j.jneumeth.2012.08.009`
- PMID: `22981419`
