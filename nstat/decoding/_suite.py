"""High-level :class:`DecoderSuite` API (moved from ``nstat/decoding.py``).

The PPLFP subpackage layout (``nstat/decoding/``) shadowed the original
flat ``nstat/decoding.py`` module, so its contents now live here and are
re-exported from :mod:`nstat.decoding` for backwards compatibility.
"""
