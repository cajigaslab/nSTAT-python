...
class History:
    ...
    def _compute_single_history(self, train: nspikeTrain, historyIndex: int | None = None, time_grid=None) -> Covariate:
        if time_grid is None:
            ...
            time = np.asarray(time_grid, dtype=float).reshape(-1)
        ...
        for col, (window_start, window_stop) in enumerate(zip(self.windowTimes[:-1], self.windowTimes[1:])):
            left = time - float(window_stop)
            right = time - float(window_start)
            history[:, col] = np.searchsorted(spikes, right, side="left") - np.searchsorted(spikes, left, side="left")
    ...
    def compute_history(self, trains, historyIndex: int | None = None, time_grid=None):
        ...
        if isinstance(trains, nspikeTrain):
            return CovariateCollection([self._compute_single_history(trains, historyIndex, time_grid=time_grid)])
        if hasattr(trains, "getNST") and hasattr(trains, "numSpikeTrains"):
            covariates = [
                self._compute_single_history(trains.getNST(index), index, time_grid=time_grid)
        ...
        if isinstance(trains, Sequence) and not isinstance(trains, (str, bytes, np.ndarray)):
            covariates = [self._compute_single_history(train, index, time_grid=time_grid) for index, train in enumerate(trains, start=1)]
            return CovariateCollection(covariates)
