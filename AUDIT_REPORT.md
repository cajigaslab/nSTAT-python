...
| M5 | Analysis | Analysis.m:1063-1066 | `phiMat` via `strfind`/`~isempty` always indexes only the first coefficient in Granger causality | HIGH (upstream: [#51](https://github.com/cajigaslab/nSTAT/issues/51) — Closed 2026-06-12) |
...
| M10 | SignalObj | SignalObj.m | `autocorrelation` typo: uses `crosscor` variable name | LOW (upstream: [#52](https://github.com/cajigaslab/nSTAT/issues/52) — Closed 2026-06-12: maintainer verified fix in master HEAD (80a54fa) — `crosscor` is now `crosscorr`) |
| M11 | SignalObj | SignalObj.m | `times`/`rdivide` operator aliasing bug | LOW (upstream: [#53](https://github.com/cajigaslab/nSTAT/issues/53) — Closed 2026-06-12) |
...
| # | Class | File:Line | Description | Severity |
|---|---|---|---|---|
| M14 | SignalObj | SignalObj.m:1342-1353 | `resample` skips when `sampleRate` unchanged but `setMinTime`/`setMaxTime` can mutate the time window post-construction, making the equality check stale.  Length mismatches surface in `makeCompatible` chains. | MEDIUM (upstream: [#54](https://github.com/cajigaslab/nSTAT/issues/54) — Closed 2026-06-12) |
| M15 | nstColl | nstColl.m:178-180 | `getFieldVal`: pre-increments `cnt` before `neuronNumbers(cnt) = ...`, so the index-array is off by one relative to `fieldVal`. | HIGH (upstream: [#55](https://github.com/cajigaslab/nSTAT/issues/55) — Closed 2026-06-12) |
| M16 | nstColl | nstColl.m:397-399 | `getNSTnameFromInd`: bounds check `ind > 0 && nstCollObj.numSpikeTrains` lacks the `ind <= numSpikeTrains` clause — author forgot to write the comparison.  Out-of-bounds indices fall through silently. | MEDIUM (upstream: [#56](https://github.com/cajigaslab/nSTAT/issues/56) — Closed 2026-06-12) |
...
| M22 | DecodingAlgorithms | Python `decoding_algorithms.py:2059, 2277` (Python-side bug; MATLAB DA.m is correct) | Original Python port of `PPHybridFilterLinear` time recursion read `MU_u[:, time_index]` (current step, always 0 since not yet written) instead of `MU_u[:, time_index - 1]` (previous step).  Effect: model probability column sums were NOT 1; only the first column got a non-zero update.  MATLAB DA.m does the right thing per gold fixtures.  **Python `decoding_algorithms.py:2059, 2277` fixed to use `time_index - 1`** (matching MATLAB), aligning with the standalone correctness test which expects probability sums = 1.  Discovered + fixed 2026-06-11 during v0.5.0 subsystem 4 deep-dive. | RESOLVED |
