...
def test_extras_gallery_html_exists_and_covers_every_manifest_entry() -> None:
    """``docs/extras_gallery.html`` ships as a self-contained showcase of
    every ``examples/extras/*_demo.py`` entry in
    ``examples/extras/manifest.yml``.  Auto-generated by
    ``tools/extras_build/build_extras_gallery.py``; lives in
    ``html_extra_path`` so the page publishes under the site root.
    """
    import yaml

    showcase = REPO_ROOT / "docs" / "extras_gallery.html"
    assert showcase.exists(), "docs/extras_gallery.html missing"

    text = showcase.read_text(encoding="utf-8")
    manifest_path = REPO_ROOT / "examples" / "extras" / "manifest.yml"
    manifest = yaml.safe_load(manifest_path.read_text(encoding="utf-8")) or {}
    manifest_demo_ids = {row["demo_id"] for row in manifest.get("extras", [])}
    for demo_id in manifest_demo_ids:
        assert f'id="{demo_id}"' in text, (
            f"extras_gallery.html missing section anchor #{demo_id}"
        )

    descriptions_path = (
        REPO_ROOT / "tools" / "extras_build" / "extras_descriptions.yml"
    )
    assert descriptions_path.exists(), (
        "tools/extras_build/extras_descriptions.yml missing — "
        "extras_gallery.html depends on it for per-figure prose."
    )
    descriptions = yaml.safe_load(descriptions_path.read_text(encoding="utf-8")) or {}
    described_demo_ids = {row["demo_id"] for row in descriptions.get("extras", [])}
    assert described_demo_ids == manifest_demo_ids, (
        "extras_descriptions.yml demo_ids must match examples/extras/manifest.yml "
        "exactly. Manifest − described = "
        f"{sorted(manifest_demo_ids - described_demo_ids)}; "
        f"described − manifest = {sorted(described_demo_ids - manifest_demo_ids)}"
    )

    conf = (REPO_ROOT / "docs" / "conf.py").read_text(encoding="utf-8")
    assert "extras_gallery.html" in conf, (
        "docs/conf.py must list extras_gallery.html under "
        "html_extra_path so Sphinx copies it into the published Pages site."
    )
...
def test_galleries_index_html_links_each_category_page() -> None:
    ...
    assert "extras_gallery.html" in text, (
        "galleries.html must link to extras_gallery.html"
    )
    conf = (REPO_ROOT / "docs" / "conf.py").read_text(encoding="utf-8")
