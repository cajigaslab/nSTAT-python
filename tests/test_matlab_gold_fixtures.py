...
def test_signalobj_matches_matlab_gold_fixture() -> None:
    ...
    signal_1 = signal.getSubSignal(0)
    ...
    xcorr = signal.getSubSignal(0).xcorr(signal.getSubSignal(1), int(_scalar(payload, "xcorr_maxlag")))
...
def test_nstcoll_matches_matlab_gold_fixture() -> None:
    ...
    n1 = nspikeTrain(_vector(payload, "firstSpikeTimes"), "1", 0.1, 0.0, 0.5, "time", "s", "spikes", "spk", -1)
    n2 = nspikeTrain(_vector(payload, "secondSpikeTimes"), "2", 0.1, 0.0, 0.5, "time", "s", "spikes", "spk", -1)
    coll = nstColl([n1, n2])
    ...
    assert coll.getNSTnameFromInd(0) == _string(payload, "nstNameFromInd1")
...
def test_analysis_fit_surface_matches_matlab_gold_fixture() -> None:
    ...
    residual = fit.computeFitResidual(0)
...
def test_cif_gamma_scaled_evals_match_matlab_gold_fixture() -> None:
    ...
    nst = nspikeTrain_cls(spike_times, "n1", 1.0 / binwidth, 0.0, 1.0)
    cif = CIF(
        beta=beta_vec,
        Xnames=["stim1", "stim2"],
        stimNames=["stim1", "stim2"],
