...
def test_helpfile_notebooks_are_code_only_and_cell_counts_match_sections() -> None:
    ...
    for row in rows:
        topic = str(row["topic"])
        notebook_path = Path(str(row["notebook_path"]))
        ...
        section_count = int(row["matlab_section_count"])
        cell_count = int(row["python_cell_count"])
        ...
        first_cell = str(nb.cells[0].source)
        assert "ensure_example_data" in first_cell, f"{topic}: missing data bootstrap in first cell"
        assert "DATA_DIR = ensure_example_data" in first_cell, f"{topic}: DATA_DIR bootstrap missing"
...
def test_helpfile_section_counts_match_matlab_when_available() -> None:
    ...
    for row in rows:
        topic = str(row["topic"])
        helpfile_rel = str(row["matlab_helpfile_path"])
        ...
        actual = int(row["matlab_section_count"])
